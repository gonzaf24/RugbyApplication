...
import { EDIT_POST } from "../../mutations/PostsMutation";
...
export const EditPostModal = ({
  ...
  const [urlDeleteFoto, setUrlDeleteFoto] = useState();
  ...
  const publicar = async (input) => {
    try {
      let { data } = await editPostNote({
        variables: { input },
      });
      if (data) {
        post = await { ...post, urlFotoPost, visibilidad, contenidoPost };
        let foundIndex = postsList.findIndex((x) => x.uid == post.uid);
        postsList[foundIndex] = post;
        setLoading(false);
        handleCloseEditPost();
      }
    } catch (error) {
      ...
      console.log(
        "hay error en editPostNote! " + JSON.stringify(error.message)
      );
    }
  };
  ...
  const onSubmitPubish = async () => {
    ...
    try {
      if (deleteFoto) {
        let filename = urlDeleteFoto.substring(
          urlDeleteFoto.lastIndexOf("/") + 1
        );
            ...
            <CardHeader
              ...
              action={
                <IconButton aria-label="settings">
                  <CloseIcon onClick={() => handleCloseEditPost()} />
                </IconButton>
              }
              title={nombreUser}
              subheader={
                ...
                >
                  ...
                  >
                    <CancelIcon
                      ...
                      onClick={() => {
                        setUrlDeleteFoto(urlFotoPost), setFileToUpload("");
                        setDeleteFoto(true), setUrlFotoPost("");
                      }}
                    />
                  </IconButton>
