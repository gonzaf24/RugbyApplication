...
import { EDIT_MATCH_RESULT } from "../../mutations/ResultsMatchsMutation";
...
export const EditMatchResultModal = ({
  openEditMatchResult,
  setOpenEditMatchResult,
  fotoUser,
  nombreUser,
  matchResult,
  matchsResultsList,
  ...
  const { lenguaje, user, postedEffect, setPostedEffect } = useContext(Context);

  const [openPublicPrivate, setOpenPublicPrivate] = useState();
  const [editMatchResult] = useMutation(EDIT_MATCH_RESULT);
  ...
  const [fechaMatch, setFechaMatch] = useState(matchResult.matchDate);
  ...
  const [score1, setScore1] = useState(matchResult.scoreTeam1);
  const [score2, setScore2] = useState(matchResult.scoreTeam2);

  const [pais, setPais] = useState(matchResult.paisMatch);
  const [paisTeam1, setPaisTeam1] = useState(matchResult.paisSelectionTeam1);
  const [paisTeam2, setPaisTeam2] = useState(matchResult.paisSelectionTeam2);
  ...
  const [value, setValue] = useState(() => {
    if (matchResult.matchType === "NATIONAL") {
      return 0;
    } else if (matchResult.matchType === "INTERNATIONAL") {
      return 1;
    } else if (matchResult.matchType === "SELECTIONS") {
      return 2;
    }
  });
  ...
  const publicar = async (input) => {
    try {
      let { data } = await editMatchResult({
        variables: { input },
      });
      if (data) {
        console.log("matchResult ...  " + JSON.stringify(matchResult));
        console.log("input ...  " + JSON.stringify(input));
        matchResult = await { ...matchResult, ...input };
        let foundIndex = await matchsResultsList.findIndex(
          (x) => x.uid == matchResult.uid
        );
        console.log("el editado es : " + JSON.stringify(matchResult));
        matchsResultsList[foundIndex] = matchResult;
        setLoading(false);
  ...
  const onSubmitEditMatchResult = async (event) => {
    console.log(" onSubmitEditMatchResult ");
            ...
            <div className={classes.root}>
              ...
              <TabPanel value={value} index={0} dir={theme.direction}>
                <form
                  ...
                  onSubmit={onSubmitEditMatchResult}
                >
